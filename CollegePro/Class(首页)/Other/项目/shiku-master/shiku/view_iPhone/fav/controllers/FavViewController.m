...
{
    FavTableViewCell *fcell=(FavTableViewCell *)cell;
...
-(void)favTableViewCellDelTaped:(GOODS *)product
{
    ...
    [[self.backend requestRemoveUserFavItems2:items]
...
- (void(^)(RACTuple *))didUpdate
{
    ...
    return ^(RACTuple *parameters) {
        ...
        if (rs.success) {
            [self.view showHUD:@"取消成功" afterDelay:2];
            [self loadData];
        }
        else{
